...
Each entry includes a `locs` array with source positions. Each location has `start` and `end` positions containing `line` (1-based), `column` (1-based), and `offset` (0-based character offset from file start).
