...
// runtime representation; the dollar-prefixed string keys are phantom-only and
// exist so the result-inference layer (`ParseObjectSelection`) can read out the
// inferred result `T` and the optional flag via conditional-type inference.
// The leading space prevents accidental autocomplete/user access.
