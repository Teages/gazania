 ...
 * directly (e.g. `$ => $`). Pipeline: `.args()` -> `.withDirective()` (terminal).
 */
export interface ScalarFieldDollar<Output, Args = Record<string, never>>
  ...
  withDirective: <U extends DirectiveInput[]>(...directives: U) =>
  HasSkipDirective<U> extends true
    ? TypedScalarSelection<true>
    : TypedScalarSelection<false>
}

// After .args(): no more args/enum, only withDirective or return directly.
 ...
 * Pipeline: `.args()` -> `.withDirective()` -> `.select()` (terminal).
...
// After .args(): no more args/enum, only withDirective or select.
...
// After .withDirective(): only .select() remains.
